...
// src/usecases/item-category/create-amount-category-usecase.spec.ts
...
var convertRGBToHEXColor = (rgb) => {
  const [r, g, b2] = rgb.replace("rgb(", "").replace(")", "").split(",");
  return "#" + [Number(r), Number(g), Number(b2)].map((x) => x.toString(16).padStart(2, "0")).join("");
};
...
describe("create item category usecase", () => {
  ...
  test("must pass", () => __async(exports, null, function* () {
    const usecase = new CreateItemCategoryUseCase(repository);
    const result = yield usecase.execute({ name: "test", color: "#fff" });
  ...
  test("cannot pass if name already exists", () => {
    ...
    const usecase = new CreateItemCategoryUseCase(repositoryClone);
    globalExpect(() => __async(exports, null, function* () {
      yield usecase.execute({ name: "test", color: "#fff" });
  ...
  test("should throw an error if the database schema is invalid", () => {
    ...
    repositoryClone.create = (name) => Promise.resolve({
      name
    });
    const usecase = new CreateItemCategoryUseCase(repositoryClone);
    globalExpect(() => __async(exports, null, function* () {
  ...
  test("cannot pass because the name has less than 3 character", () => {
    ...
    globalExpect(() => __async(exports, null, function* () {
      try {
        yield usecase.execute({ name: "", color: "#fff" });
      } catch (error) {
        if (error instanceof UpError) {
          globalExpect(error.message).toBe("The minimum name length is 3");
        }
        throw "test pass";
      }
  ...
  test("cannot pass because the name has more than 30 character", () => {
    ...
    globalExpect(() => __async(exports, null, function* () {
      try {
        yield usecase.execute({ name: "this_text_has_to_more_at_30_characters", color: "#fff" });
      } catch (error) {
        if (error instanceof UpError) {
  ...
  test("cannot pass because the color is not a HEX or RGB color", () => {
    ...
    globalExpect(() => __async(exports, null, function* () {
      try {
        yield usecase.execute({ name: "test", color: "hsl(100%, 0%, 53%)" });
      } catch (error) {
        if (error instanceof UpError) {
          globalExpect(error.message).toBe("The color must be a valid HEX or RGB code");
        }
        throw "test pass";
      }
    })).rejects.toThrow("test pass");
