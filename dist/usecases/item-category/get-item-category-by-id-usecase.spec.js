...
// src/usecases/item-category/get-item-category-by-id-usecase.spec.ts
...
describe("get item category by id usecase", () => {
  ...
  test("must pass", () => __async(exports, null, function* () {
    const usecase = new GetItemCategoryByIdUseCase(repository);
  ...
  test("should throw an error if the database schema is invalid", () => {
    ...
    const usecase = new GetItemCategoryByIdUseCase(repositoryClone);
  ...
  test("should throw an error if the item category not exists with this id", () => {
    const repositoryClone = __spreadValues({}, repository);
    repository.delete = (id) => Promise.resolve(false);
    const usecase = new GetItemCategoryByIdUseCase(repositoryClone);
    globalExpect(() => __async(exports, null, function* () {
      yield usecase.execute("id_invalido");
    })).rejects.toThrow(UpError);
