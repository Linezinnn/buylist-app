...
// src/usecases/item/check-item-usecase.spec.ts
...
describe("check item usecase", () => {
  ...
  const anotherUUID = (0, import_crypto.randomUUID)();
  const itemMock = {
    name: "TESTE",
    amount: 200,
    isChecked: true,
  ...
  test("must pass", () => {
    const usecase = new CheckItemUseCase(repository);
    globalExpect(() => __async(exports, null, function* () {
      const result = yield usecase.execute(uuid, { checked: true });
      globalExpect(result).toStrictEqual(itemMock);
    })).not.toThrow();
  });
  test("cannot pass if the id is invalid", () => {
    const usecase = new CheckItemUseCase(repository);
    globalExpect(() => __async(exports, null, function* () {
      yield usecase.execute("id_invalido", { checked: true });
    })).rejects.toThrow(UpError);
  });
  test("should throw an error if the item not exists with this id", () => {
    let repositoryClone = __spreadValues({}, repository);
    repositoryClone.checkItem = () => Promise.resolve(null);
    const usecase = new CheckItemUseCase(repositoryClone);
    globalExpect(() => __async(exports, null, function* () {
      yield usecase.execute(anotherUUID, { checked: true });
    })).rejects.toThrow(UpError);
  });
  test("should throw an error if the database schema is invalid", () => {
    const repositoryClone = __spreadValues({}, repository);
    repositoryClone.checkItem = (name) => Promise.resolve({
      name
    });
    const usecase = new CheckItemUseCase(repositoryClone);
    globalExpect(() => __async(exports, null, function* () {
      yield usecase.execute(uuid, { checked: true });
    })).rejects.toThrow(UpError);
  });
});
