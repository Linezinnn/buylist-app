...
// src/usecases/item/create-amount-category-usecase.spec.ts
...
describe("create item usecase", () => {
  ...
  const amountCategoryRepository = {
    getById(id) {
      return Promise.resolve({
        name: "item_category",
        id: uuid,
  ...
  test("must pass", () => __async(exports, null, function* () {
    const usecase = new CreateItemUseCase(
      itemRepository,
      amountCategoryRepository,
  ...
  test("cannot pass if name already exists", () => {
    let repositoryClone = __spreadValues({}, itemRepository);
    repositoryClone.getByName = (name) => {
      return Promise.resolve(itemMock);
    };
    const usecase = new CreateItemUseCase(
      repositoryClone,
  ...
  test("cannot pass if amount category not exists", () => {
    let repositoryClone = __spreadValues({}, amountCategoryRepository);
    repositoryClone.getById = (id) => Promise.resolve(null);
    const usecase = new CreateItemUseCase(
      itemRepository,
      repositoryClone,
      itemCategoryRepository
  ...
  test("cannot pass if item category not exists", () => {
    let repositoryClone = __spreadValues({}, itemCategoryRepository);
    repositoryClone.getById = (id) => Promise.resolve(null);
    const usecase = new CreateItemUseCase(
      itemRepository,
      amountCategoryRepository,
    ...
    globalExpect(() => __async(exports, null, function* () {
      try {
        ...
        if (error instanceof UpError) {
          globalExpect(error.message).toBe("The maximium name length is 30");
        }
        throw "test pass";
      }
