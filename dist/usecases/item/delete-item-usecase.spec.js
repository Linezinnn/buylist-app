...
// src/usecases/item/delete-item-usecase.spec.ts
...
describe("delete item usecase", () => {
  ...
  test("must pass", () => {
    ...
    globalExpect(() => {
      usecase.execute((0, import_crypto.randomUUID)());
    }).not.toThrow();
  });
  ...
  test("should throw an error if the item not exists with this id", () => {
    ...
    repositoryClone.delete = (id) => Promise.resolve(false);
    const usecase = new DeleteItemUseCase(repositoryClone);
    globalExpect(() => __async(exports, null, function* () {
      yield usecase.execute("id_invalido");
    })).rejects.toThrow(UpError);
  });
