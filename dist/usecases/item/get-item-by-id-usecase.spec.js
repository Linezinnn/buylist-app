...
// src/usecases/item/get-item-by-id-usecase.spec.ts
...
describe("get item by id usecase", () => {
  ...
  test("must pass", () => __async(exports, null, function* () {
    const usecase = new GetItemByIdUseCase(repository);
    const result = yield usecase.execute(uuid);
  ...
  test("should throw an error if the database schema is invalid", () => {
    ...
    repositoryClone.getById = (name) => Promise.resolve({
      name
    });
    const usecase = new GetItemByIdUseCase(repositoryClone);
    globalExpect(() => __async(exports, null, function* () {
      yield usecase.execute(uuid);
    })).rejects.toThrow(UpError);
  });
  test("cannot pass if the id is invalid", () => {
    const usecase = new GetItemByIdUseCase(repository);
    globalExpect(() => __async(exports, null, function* () {
      yield usecase.execute("id_invalido");
    })).rejects.toThrow(UpError);
  });
