...
function inspectTypedArray(array2, options) {
  ...
  for (let i = 0; i < array2.length; i++) {
    const string4 = `${options.stylize(truncate(array2[i], options.truncate), "number")}${i === array2.length - 1 ? "" : ", "}`;
...
var MockDate = class _MockDate extends RealDate {
  constructor(y2, m2, d2, h, M2, s, ms) {
    ...
    switch (arguments.length) {
      case 0:
        if (now2 !== null)
          date2 = new RealDate(now2.valueOf());
        else
          date2 = new RealDate();
        break;
      case 1:
      ...
      default:
        ...
        s = s || 0;
        ms = ms || 0;
        date2 = new RealDate(y2, m2, d2, h, M2, s, ms);
        break;
    }
    Object.setPrototypeOf(date2, _MockDate.prototype);
    return date2;
  }
};
  ...
  function createIntl() {
    ...
    ClockIntl.DateTimeFormat = function(...args) {
      ...
      ["format", "formatToParts"].forEach((method) => {
        formatter[method] = function(date2) {
          return realFormatter[method](date2 || ClockIntl.clock.now);
        };
      });
      return formatter;
    };
...
// src/utils/validations/schemas/item-category-schema.test/item-category-dto-get-schema.ts
...
// src/utils/validations/schemas/item-category-schema.test/item-category-dto-get-schema.ts
function TESTItemCategoryDTOGetSchema() {
...
// src/utils/validations/schemas/item-category-schema.test/item-category-dto-mutation-schema.ts
var import_zod3 = require("zod");
function TESTItemCategoryDTOMutationSchema() {
  const testData = {
    name: "item_category",
    color: "#121212"
  };
  describe("item category dto mutation schema", () => {
    test("validation needs to pass", () => {
      ...
      globalExpect(validatedData).toHaveProperty("name", "item_category");
      globalExpect(validatedData).toHaveProperty("color", "#121212");
    });
    ...
    describe("the color must be a HEX or RGB code", () => {
      ...
      test("RGB", () => {
        ...
        globalExpect(validatedData).toHaveProperty("color", "rgb(255,255,255)");
      });
      test("HEX", () => {
        testDataClone.color = "#FFF";
        const validatedData = ItemCategoryDTOMutationSchema.parse(testDataClone);
        globalExpect(validatedData).toHaveProperty("color", "#FFF");
      });
    });
    test("color cannot be a code other than HEX or RGB", () => {
      ...
      globalExpect(() => {
        try {
          ...
          if (error instanceof import_zod3.ZodError) {
            const { message } = error.issues[0];
            globalExpect(message).toEqual("The color must be a valid HEX or RGB code");
            throw "test pass";
          }
        }
      }).toThrow("test pass");
...
// src/utils/validations/schemas/item-category-schema.test/item-category-response.schema.ts
var import_crypto2 = require("crypto");
var import_zod4 = require("zod");
function TESTItemCategoryResponseSchema() {
  ...
  describe("item category response schema", () => {
    test("the validation needs to pass", () => {
      ...
      globalExpect(validatedData).toHaveProperty("createdAt", createdAt);
    });
    test("the name cannot have a name length greater than 30 characters", () => {
      const testDataClone = __spreadValues({}, testData);
      testDataClone.name = "this_name_have_a_length_greater_than_30_characters";
      globalExpect(() => {
        try {
          ...
          if (error instanceof import_zod4.ZodError) {
            const { message } = error.issues[0];
            globalExpect(message).toEqual("The maximum name length is 30");
            throw "test pass";
          }
        }
      }).toThrow("test pass");
      ...
      test("incorrect value", () => {
        globalExpect(() => {
          try {
            ...
            if (error instanceof import_zod4.ZodError) {
              const { message } = error.issues[0];
              globalExpect(message).toEqual("The color must be a HEX code");
              throw "test pass";
            }
          }
        }).toThrow("test pass");
...
// src/utils/validations/schemas/item-category-schema.test/index.spec.ts
