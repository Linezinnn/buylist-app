...
function truncate(string6, length, tail = truncator) {
  string6 = String(string6);
  const tailLength = tail.length;
  const stringLength = string6.length;
  if (tailLength > length && stringLength > tailLength) {
    return tail;
  }
  if (stringLength > length && stringLength > tailLength) {
    return `${string6.slice(0, length - tailLength)}${tail}`;
  }
  return string6;
}
function inspectList(list, options, inspectItem, separator = ", ") {
  ...
  for (let i = 0; i < size; i += 1) {
    ...
    const string6 = peek || inspectItem(value, options) + (last ? "" : separator);
    const nextLength = output.length + string6.length;
...
function inspectTypedArray(array2, options) {
  ...
  for (let i = 0; i < array2.length; i++) {
    const string6 = `${options.stylize(truncate(array2[i], options.truncate), "number")}${i === array2.length - 1 ? "" : ", "}`;
    options.truncate -= string6.length;
...
function inspectDate(dateObject, options) {
  ...
  const date4 = split[0];
  return options.stylize(`${date4}T${truncate(split[1], options.truncate - date4.length - 1)}`, "date");
}
var init_date = __esm({
  "node_modules/loupe/lib/date.js"() {
    "use strict";
...
function inspectString(string6, options) {
  if (stringEscapeChars.test(string6)) {
    string6 = string6.replace(stringEscapeChars, escape);
  }
  return options.stylize(`'${truncate(string6, options.truncate - 2)}'`, "string");
}
...
var init_magic_string_es = __esm({
  "node_modules/magic-string/dist/magic-string.es.mjs"() {
    ...
    MagicString = class _MagicString {
      constructor(string6, options = {}) {
        const chunk = new Chunk(0, string6.length, string6);
        Object.defineProperties(this, {
          original: { writable: true, value: string6 },
        ...
        this.byEnd[string6.length] = chunk;
      }
      ...
      _replaceString(string6, replacement) {
        const { original } = this;
        const index = original.indexOf(string6);
        if (index !== -1) {
          this.overwrite(index, index + string6.length, replacement);
        }
        return this;
      }
      ...
      _replaceAllString(string6, replacement) {
        const { original } = this;
        const stringLength = string6.length;
        for (let index = original.indexOf(string6); index !== -1; index = original.indexOf(string6, index + stringLength)) {
...
function addExtraLineBreaks(string6) {
  return string6.includes("\n") ? `
${string6}
` : string6;
}
function removeExtraLineBreaks(string6) {
  return string6.length > 2 && string6.startsWith("\n") && string6.endsWith("\n") ? string6.slice(1, -1) : string6;
}
...
function normalizeNewlines(string6) {
...
var MockDate = class _MockDate extends RealDate {
  constructor(y2, m2, d2, h, M2, s, ms) {
    ...
    switch (arguments.length) {
      case 0:
        if (now2 !== null)
          date4 = new RealDate(now2.valueOf());
        else
          date4 = new RealDate();
        break;
      case 1:
        date4 = new RealDate(y2);
        break;
      default:
        ...
        s = s || 0;
        ms = ms || 0;
        date4 = new RealDate(y2, m2, d2, h, M2, s, ms);
        break;
    }
    Object.setPrototypeOf(date4, _MockDate.prototype);
    return date4;
  }
};
...
function mockDate(date4) {
  const dateObj = new RealDate(date4.valueOf());
  if (Number.isNaN(dateObj.getTime()))
    throw new TypeError(`mockdate: The time set is an invalid date: ${date4}`);
...
${job.error.stack.split("\n").slice(matchedLineIndex + 1).join("\n")}`;
  ...
  function createDate() {
    function ClockDate(year, month, date4, hour, minute, second, ms) {
      ...
      switch (arguments.length) {
        ...
        case 3:
          return new NativeDate(year, month, date4);
        case 4:
          return new NativeDate(year, month, date4, hour);
        case 5:
          return new NativeDate(year, month, date4, hour, minute);
        case 6:
  ...
  function createIntl() {
    ...
    ClockIntl.DateTimeFormat = function(...args) {
      ...
      ["format", "formatToParts"].forEach((method) => {
        formatter[method] = function(date4) {
          return realFormatter[method](date4 || ClockIntl.clock.now);
        };
      });
      return formatter;
    };
...
To automatically clean-up native timers, use \`shouldClearNativeTimers\`.`
  ...
  function hijackMethod(target, method, clock) {
    ...
    if (method === "Date") {
      const date4 = mirrorDateProperties(clock[method], target[method]);
      target[method] = date4;
    } else if (method === "Intl") {
...
function createVitest() {
  ...
  const utils = {
    ...
    setSystemTime(time) {
      const date4 = time instanceof Date ? time : new Date(time);
      _mockedDate = date4;
      _timers.setSystemTime(date4);
      return utils;
    },
    getMockedSystemTime() {
      return _mockedDate;
...
// src/utils/validations/schemas/item-schema.test/item-dto-get-schema.ts
...
function TESTItemDTOGetSchema() {
  ...
  describe("item dto get schema", () => {
    test("the validation needs to pass", () => {
      ...
      globalExpect(validatedData).toStrictEqual({ id: uuid });
    });
    test("unable to pass because ID is invalid", () => {
      let testDataClone = __spreadValues({}, testData);
      testDataClone.id = "invalid_id";
      globalExpect(() => {
        try {
          ...
          if (error instanceof import_zod4.ZodError) {
            ...
            globalExpect(message).toEqual("This uuid is invalid");
            throw "test pass";
          }
        }
      }).toThrow("test pass");
...
// src/utils/validations/schemas/item-schema.test/item-dto-mutation-schema.ts
var import_zod5 = require("zod");
var import_crypto2 = require("crypto");
function TESTItemDTOMutationSchema() {
  ...
  describe("item dto mutation schema", () => {
    test("validation needs to pass", () => {
      ...
      globalExpect(validatedData).toHaveProperty("name", "ITEM");
      globalExpect(validatedData).toHaveProperty("amountCategoryId", uuid);
      globalExpect(validatedData).toHaveProperty("itemCategoryId", uuid);
      globalExpect(validatedData).toHaveProperty("amount", 200);
    });
    test("the name cannot have a name length greater than 30 characters", () => {
      const testDataClone = __spreadValues({}, testData);
      ...
      globalExpect(() => {
        try {
          ...
          if (error instanceof import_zod5.ZodError) {
            const { message } = error.issues[0];
            globalExpect(message).toEqual("The maximium name length is 30");
            throw "test pass";
          }
        }
      }).toThrow("test pass");
    ...
    test("the name cannot have a name length at least than 3 characters", () => {
      ...
      globalExpect(() => {
        try {
          ...
          if (error instanceof import_zod5.ZodError) {
            const { message } = error.issues[0];
            globalExpect(message).toEqual("The minimum name length is 3");
            throw "test pass";
          }
        }
      }).toThrow("test pass");
    ...
    test("unable to pass because ID is invalid", () => {
      let testDataClone = __spreadValues({}, testData);
      testDataClone.amountCategoryId = "invalid_id";
      globalExpect(() => {
        try {
          ItemDTOMutationSchema.parse(testDataClone);
        } catch (error) {
    ...
    test("the amount must be at least 1", () => {
      ...
      globalExpect(() => {
        try {
          ...
          if (error instanceof import_zod5.ZodError) {
            const { message } = error.issues[0];
            globalExpect(message).toEqual("The minimum amount is 1");
            throw "test pass";
          }
        }
      }).toThrow("test pass");
    ...
    test("the amount cannot be greater than 100.000", () => {
      ...
      globalExpect(() => {
        try {
          ...
          if (error instanceof import_zod5.ZodError) {
            const { message } = error.issues[0];
            globalExpect(message).toEqual("The maximium amount is 100.000");
            throw "test pass";
          }
        }
      }).toThrow("test pass");
...
// src/utils/validations/schemas/item-schema.test/item-response.schema.ts
var import_crypto3 = require("crypto");
var import_zod6 = require("zod");
function TESTItemResponseSchema() {
  const uuid = (0, import_crypto3.randomUUID)();
  ...
  describe("item response schema", () => {
    test("the validation needs to pass", () => {
      ...
      globalExpect(validatedData).toHaveProperty("name", "NAME_TEST");
      globalExpect(validatedData).toHaveProperty("amount", 200);
      globalExpect(validatedData).toHaveProperty("isChecked", false);
      globalExpect(validatedData).toHaveProperty("amountCategoryId", uuid);
      globalExpect(validatedData).toHaveProperty("itemCategoryId", uuid);
      globalExpect(validatedData).toHaveProperty("id", uuid);
      globalExpect(validatedData).toHaveProperty("createdAt", date4);
      globalExpect(validatedData).toHaveProperty("updatedAt", date4);
    });
    test("the name cannot have a name length greater than 30 characters", () => {
      const testDataClone = __spreadValues({}, testData);
      testDataClone.name = "this_name_have_a_length_greater_than_30_characters";
    ...
    test("cannot pass because the date is not valid", () => {
      let testDataClone = __spreadValues({}, testData);
      testDataClone.createdAt = "incorrect_date";
      globalExpect(() => {
        ItemResponseSchema.parse(testDataClone);
      }).toThrow(import_zod6.ZodError);
    });
  });
}

// src/utils/validations/schemas/item-schema.test/index.spec.ts
